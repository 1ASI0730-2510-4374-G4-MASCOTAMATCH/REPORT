...
| HU03 | Compra de planes disponibles | Como visitante, quiero ver los planes disponibles y sus beneficios para elegir el de preferencia propia   | Escenario 1: Se muestran los distintos planes disponibles. **Given** ingreso a la sección de planes, **When** visualizo los planes disponibles con sus respectivos beneficios y precios, **Then** elijo uno y le doy al botón “Comprar”. | EPIC02 |
| HU04 | Selección de método de pago | Como usuario, quiero elegir método de pago (Yape, tarjeta, PayPal) desde la vista de planes. | Escenario 1: Desde un botón en la tarjeta accedo al pago. <br> **Given** que estoy en la página de planes o en citas, **When** quiero acceder a un pago, le doy al botón "Comprar", **Then** puedo elegir métodos como Yape, tarjeta o PayPal y confirmar el pago. | EPIC02 |
| HU05 | Historial de pagos |Como usuario, quiero consultar un historial de pagos anteriores, para tener registro de mis transacciones realizadas. | Escenario 1: Luego de acceder a un pago se registra debajo un apartado del historial de pagos <br> **Given** que he pagado una cita o plan, **When** visualizo que debajo se registra el pago en un historial **Then** puedo verificar los datos del pago desde esa boleta. | EPIC02 |
| HU06 | Visualización de medicamentos comprados | Como usuario quiero ver una lista de mis medicamentos, para tener control sobre la atención de mi mascota. | Escenario 1: Se muestra una lista con los medicamentos ordenados por compra reciente<br>**Given** estoy autenticado, **When** accedo al historial", **Then** visualizo todos los medicamentos registrados. | EPIC05 |
| HU07 | Detalle de medicamentos | Como usuario, quiero ver el detalle del medicamento que adquirí, para revisar la información específica. | Escenario 1: Al visualizar la lista de medicamentos se muestra el detalle sobre su efecto detallado. <br>**Given** estoy en el historial, **When** visualizo los medicamentos comprados, **Then** veo toda la información relacionada con la enfermedad que combate o efectos que realiza. | EPIC05 |
| HU08 | Historial de pedidos  | Como proveedor de cuidado animal,  quiero consultar el historial de todos los medicamentos pedidos, para hacer seguimiento del servicio ofrecido. | Escenario 1: Se muestra una lista completa de pedidos. <br>**Given** estoy autenticado, **When** accedo al historial, **Then** puedo ver toda¿os las pedidos de mis clientes con sus respectivos detalles. | EPIC05 |
| HU09 | Comprar medicina | Como usuario, quiero comprar medicinas para el cuidado de mi mascota | Escenario 1: Quiero comprar medicina para mi mascota. <br> **Given** dado estoy viendo las medicinas, **When** agrego la cantidad que necesito **Then** se registra la compra en el historial | EPIC05 |
| HU10 | Visualización de servicios ofrecidos | Como visitante, quiero ver tarjetas de servicios con iconos, nombre y descripción. Para poder comprender con facilidad | Escenario 1: Se muestran 4 tarjetas de servicios. <br> **Given** ingreso a la sección de Services, **When** se carga la página, **Then** visualizo las 4 tarjetas con información clara sobre cada uno de los servicios. | EPIC06 |
| HU11 | Consulta de información en FAQ | Como visitante, quiero ver una sección con preguntas frecuentes organizadas para resolver dudas. | Escenario 1: Quiero acceder a una sección de preguntas frecuentes (FAQ) <br> **Given** accedo a la sección de FAQ, **When** se carga la página, **Then** visualizo al menos 6 preguntas frecuentes. | EPIC06 |
| HU12 | Registro de mascotas | Como usuario, quiero ingresar datos de mi mascota, para tener la información registrada. | Escenario 1: El usuario accede a mascotas y puede s <br>**Given** estoy autenticado, **When** accedo a "Mi perfil" y edito mis datos, **Then** los cambios se guardan correctamente. | EPIC03 |
...
| TS03 | Gestión de mascotas | Como desarrollador, quiero permitir que el usuario vea, agregue, elimine y seleccione mascotas en tarjetas, para que gestione fácilmente su información. | Escenario 1: El usuario accede a un formulario de registro de mascotas<br> **Given** que quiero gestionar las mascotas registradas, **When** uso las opciones de "Agregar" o "Eliminar", **Then** tengo un control de las mascotas registradas. | EPIC03 |
...
| TS05 | Compra de medicamentos | Como desarrollador, quiero mostrar medicamentos con nombre, descripción, selector de cantidad y botón “Comprar”, para que el usuario adquiera lo necesario. | Escenario 1: El usuario accede a la pestaña “Medicamentos” y ve vacunas, desparasitantes y antibióticos.<br>**Given** que accedo a mi perfil, **When** hago clic en “Historial”, **Then** veo una lista cronológica de mis servicios anteriores. | EPIC04 |
...
| TS07 | Historial de medicamentos | Como desarrollador, quiero mostrar en una tabla el historial de productos aplicados a cada mascota, para que el usuario tenga trazabilidad de los tratamientos. | Escenario 1: El usuario accede al historial y ve una tabla con columnas: ID, Nombre, Mascota, Productos. <br> **Given** que estoy autenticado, **When** hago clic en “Cerrar sesión”, **Then** mi sesión se cierra y soy redirigido al login. | EPIC05 |
| TS10  | Agendar citas | Como desarrollador, quiero registrar una cita nueva para la mascota desde un formulario.        | Escenario 1: POST a `/appointments` con mascota, proveedor, fecha y tipo. Se guarda correctamente. <br> Escenario 2: Si hay error o campo vacío, el sistema muestra mensaje y no guarda.                    | EPIC01           |
| TS15  | Obtener citas agendadas | Como desarrollador, quiero obtener desde la API la lista de citas del usuario.                 | Escenario 1: GET a `/appointments/{userId}` muestra citas en orden por fecha. <br> Escenario 2: Si no hay citas registradas, se muestra mensaje “No hay citas agendadas aún”.                              | EPIC01           |
