...
| EPIC05 | Historial de citas | **Como** visitante,<br>**Quiero** consultar el historial de citas y servicios recibidos, <br>**Para**tener un seguimiento claro de la atención brindada a mi mascota. |  | - |
...
|-------|--------------------------------------|-----------------------------------------------------------------------------------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|
| HU01 | Registro de usuarios | Como usuario del segmento objetivo Dueño de mascota/Proveedor de cuidado animal, quiero registrarme para acceder a la plataforma. | Escenario 1: Desde login se puede ir a registro. <br> **Given** estoy en la pantalla de login, **When** no me he registrado antes , **Then** veo un enlace para registrarme y accedo. <br> Escenario 2: Se ingresan las credenciales inválidas. <br> **Given** que quiero registrarme **When** ya me he registrado anteriormente, **Then** se le niega la operación | EPIC01 |
| HU02 | Inicio de sesión | Como usuario del segmento objetivo Dueño de mascota/Proveedor de cuidado animal, quiero iniciar sesión para acceder a la landing. | Escenario 1: Inicio de sesión exitoso <br> **Dado** que me registré como Dueño de mascota/Proveedor de cuidado animal, **When** ingreso mi correo y contraseña, **Then** debería acceder a la landing <br> Escenario 2: Inicio de sesión fallido **Given** que soy un médico/paciente registrado, **When** ingreso mi correo y contraseña incorrecto, **Then** se muestra un error donde te pide logearte nuevamente con las credenciales correctas | EPIC01 |
| HU03  | Visualización de servicios ofrecidos | Como visitante, quiero ver tarjetas de servicios con iconos, nombre y descripción. Para poder comprender con facilidad | Escenario 1: Se muestran 4 tarjetas de servicios (paseos, grooming, emergencias, recordatorios). <br> **Given** ingreso a la sección de Services, **When** se carga la página, **Then** visualizo las 4 tarjetas con información clara. | EPIC06 |
| HU04  | Consulta de información en FAQ | Como visitante, quiero ver una sección con preguntas frecuentes organizadas para resolver dudas.     | Escenario 1: Quiero acceder a una sección de preguntas frecuentes (FAQ) <br> **Given** accedo a la sección de FAQ, **When** se carga la página, **Then** visualizo al menos 6 preguntas frecuentes. | EPIC06 |
| HU05  | Compra de planes disponibles | Como visitante, quiero ver los planes disponibles y sus beneficios para elegir el de preferencia propia   | Escenario 1: Se muestran los distintos planes disponibles. **Given** ingreso a la sección de planes, **When** visualizo los planes disponibles con sus respectivos beneficios y precios, **Then** elijo uno y le doy al botón “Comprar”. | EPIC02 |
| HU06  | Selección de método de pago | Como usuario, quiero elegir método de pago (Yape, tarjeta, PayPal) desde la vista de planes.         | Escenario 1: Desde un botón en la tarjeta accedo al pago. <br> **Given** que estoy en la página de planes o en citas, **When** quiero acceder a un pago, le doy al botón "Comprar", **Then** puedo elegir métodos como Yape, tarjeta o PayPal y confirmar el pago. | EPIC02           |
| HU07  | Historial de pagos |Como usuario, quiero consultar un historial de pagos anteriores, para tener registro de mis transacciones realizadas.         | Escenario 1: Luego de acceder a un pago se registra debajo un apartado del historial de pagos <br> **Given** que he pagado una cita o plan, **When** visualizo que debajo se registra el pago en un historial **Then** puedo verificar los datos del pago desde esa boleta. | EPIC02 |
| HU08  | Modificar datos del perfil de usuario               | Como usuario, quiero modificar mis datos personales desde mi perfil, para mantener mi información actualizada. | Escenario 1: El usuario accede a su perfil y puede editar campos como nombre, correo y dirección. <br>**Given** estoy autenticado, **When** accedo a "Mi perfil" y edito mis datos, **Then** los cambios se guardan correctamente. | EPIC01 |
| HU09  | Visualización de citas             | Como quiero ver una lista de mis próximas citas, para tener control sobre la atención de mi mascota. | Escenario 1: Se muestra una lista con citas ordenadas por fecha. <br>**Given** estoy autenticado, **When** accedo al historial", **Then** visualizo todas las citas registradas. | EPIC05 |
| HU10  | Detalle de citas             | Como visitante, quiero ver el detalle completo de una cita seleccionada, para revisar la información específica de la atención. | Escenario 1: Al visualizar la lista de citas se muestra el detalle con dueño, mascota, fecha. <br>**Given** estoy en el historial de citas, **When** selecciono una cita, **Then** veo toda la información relacionada. | EPIC05 |
| HU11  | Historial de citas             | Como proveedor de cuidado animal,  quiero consultar el historial de todas mis citas, para hacer seguimiento del servicio ofrecido. | Escenario 1: Se muestra una lista completa de citas. <br>**Given** estoy autenticado, **When** accedo al historial de citas, **Then** puedo ver todas las citas de mis clientes con sus respectivos detalles. | EPIC05 |
| HU12  |  Visualización de planes de suscripción | Como visitante, quiero ver los tres planes para comparar cual es mejor opción | Escenario 1: Cada plan tiene una descripción sobre sus beneficios y botón "Read More". <br> **Given** estoy en la página de planes, **When** observo cada plan y sus beneficios, **Then** soy redirigido a una página con más detalles de ese plan. | EPIC06 |
| HU14  | Agendar citas | Como usuario, quiero agendar citas para asignar un tiempo acordado | Escenario 1: Cada tarjeta tiene botón “Agendar”. <br> **Given** estoy viendo un servicio, **When** hago clic en "Agendar", **Then** se abre un formulario con fecha, proveedor y tipo de atención. | EPIC05 |
| TS01 | Inicio de Sesión con validación | Como desarrollador, quiero que los usuarios ingresen su correo y contraseña y reciban una validación inmediata, para evitar campos vacíos o datos incorrectos.   | Escenario 1: El usuario deja campos vacíos y el sistema muestra una alerta “Por favor ingrese su correo y contraseña.”<br>**Given** completo el formulario de cita, **When** hago clic en "Confirmar", **Then** recibo una notificación visual de éxito. <br>                                                               | EPIC01 |
| TS02          | Visualizar planes disponibles    | Como desarrollador, quiero mostrar los distintos planes (Mini,Complete,vip) con nombre, descripción y botón “Comprar”, para facilitar elección.  | Escenario 1: El usuario accede y ve los tres planes diferenciados visualmente.<br>Escenario 2: Al hacer clic en “Comprar”, se registra la elección del plan. <br> **Given** dejo campos obligatorios vacíos, **When** intento enviar, **Then** se muestra un mensaje de error en los campos correspondientes.                                                                                                                                         | EPIC02                  |
| TS03          | Gestión de mascotas              | Como desarrollador, quiero permitir que el usuario vea, agregue, elimine y seleccione mascotas en tarjetas, para que gestione fácilmente su información.          | Escenario 1: Se muestran las tarjetas con nombre y edad de cada mascota.<br> **Given** no recuerdo mi contraseña, **When** uso la opción “Olvidé mi contraseña”, **Then** recibo instrucciones por correo.                                                                                                                   | EPIC03                  |
| TS04          | Agregar nueva mascota            | Como desarrollador, quiero que el usuario pueda ingresar nombre y edad en un formulario sencillo, para registrar fácilmente a su mascota.                        | Escenario 1: El usuario accede al formulario desde el menú “Agregar Mascota”.<br> **Given** tengo una cita agendada, **When** hago clic en “Cancelar”, **Then** se elimina la cita y se muestra un mensaje de confirmación.                                                                                                                          | EPIC03                  |
| TS05          | Compra de medicamentos           | Como desarrollador, quiero mostrar medicamentos con nombre, descripción, selector de cantidad y botón “Comprar”, para que el usuario adquiera lo necesario.       | Escenario 1: El usuario accede a la pestaña “Medicamentos” y ve vacunas, desparasitantes y antibióticos.<br>**Given** accedo a mi perfil, **When** hago clic en “Historial”, **Then** veo una lista cronológica de mis servicios anteriores.                                                                       | EPIC04                  |
| TS06         | Validación de campos vacíos en compra | Como desarrollador, quiero validar que el usuario seleccione al menos una unidad de medicamento antes de hacer clic en “Comprar”, para evitar errores de compra. | Escenario 1: Si no hay unidades seleccionadas, el sistema muestra un mensaje de error.<br> **Given** estoy en mi perfil, **When** hago clic en “Editar”, **Then** puedo actualizar mis datos y guardar los cambios correctamente.	.                                                                                                  | EPIC04                  |
| TS07          | Historial de tratamientos        | Como desarrollador, quiero mostrar en una tabla el historial de productos aplicados a cada mascota, para que el usuario tenga trazabilidad de los tratamientos.   | Escenario 1: El usuario accede al historial y ve una tabla con columnas: ID, Nombre (usuario), Mascota, Producto, Descripción.<br> **Given** estoy autenticado, **When** hago clic en “Cerrar sesión”, **Then** mi sesión se cierra y soy redirigido al login.                                                                                        | EPIC05                 |
| TS08          |  Mostrar historial  | Como desarrollador, quiero permitir filtrar el historial por mascota, para que el usuario visualice solo los registros relevantes.                               | Escenario 1: El usuario selecciona una mascota de una lista desplegable y ve solo sus tratamientos.<br>**Given** he iniciado sesión correctamente, **When** veo la barra de navegación, **Then** se muestra mi nombre en la esquina superior derecha.                                                                                                           | EPIC05                  |


| ID    | Título                         | Descripción                                                                                     | Criterios de Aceptación                                                                                                                                                                                     | Epic Relacionado |
|-------|--------------------------------|-------------------------------------------------------------------------------------------------|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|------------------|
| TS09  | Servicios disponibles (GET)     | Como desarrollador, quiero conectar con la API para obtener los servicios disponibles.          | Escenario 1: Se hace un GET a `/services`. Si hay datos, se muestran como tarjetas. <br> Escenario 2: Si no hay servicios, se muestra mensaje “No hay servicios disponibles por el momento”.                 | EPIC01           |
| TS10  | Planes disponibles (GET)        | Como desarrollador, quiero conectar con la API para obtener los planes de suscripción.          | Escenario 1: Se hace GET a `/plans`. Se muestran tarjetas de planes. <br> Escenario 2: Si no hay planes, se muestra un mensaje informativo para el usuario.                                                 | EPIC02           |
| TS11  | Preguntas frecuentes (GET)      | Como desarrollador, quiero mostrar la sección FAQ conectada a la API.                          | Escenario 1: Se hace GET a `/faq`. Las preguntas y respuestas aparecen en tarjetas. <br> Escenario 2: Si no hay preguntas, se informa “No hay preguntas registradas”.                                      | EPIC03           |
| TS12  | Registro de usuarios (POST)     | Como desarrollador, quiero enviar los datos del usuario nuevo al servidor para su registro.     | Escenario 1: Se hace POST a `/register` con nombre, correo y contraseña. <br> Escenario 2: Si faltan campos o hay error, se muestra un mensaje validando los campos.                                        | EPIC05           |
| TS13  | Inicio de sesión (POST)         | Como desarrollador, quiero autenticar usuarios desde el login.                                  | Escenario 1: Se hace POST a `/login`. Si las credenciales son correctas, se accede. <br> Escenario 2: Si las credenciales son incorrectas, se muestra mensaje de error.                                     | EPIC05           |
| TS14  | Obtener perfil de usuario (GET) | Como desarrollador, quiero obtener los datos del usuario logueado.                              | Escenario 1: Se hace GET a `/users/profile`. Se muestran nombre, correo y mascotas. <br> Escenario 2: Si no hay sesión iniciada, se redirige al login.                                                      | EPIC05           |
| TS15  | Agendar citas (POST)            | Como desarrollador, quiero registrar una cita nueva para la mascota desde un formulario.        | Escenario 1: POST a `/appointments` con mascota, proveedor, fecha y tipo. Se guarda correctamente. <br> Escenario 2: Si hay error o campo vacío, el sistema muestra mensaje y no guarda.                    | EPIC01           |
| TS16  | Obtener citas agendadas (GET)   | Como desarrollador, quiero obtener desde la API la lista de citas del usuario.                 | Escenario 1: GET a `/appointments/{userId}` muestra citas en orden por fecha. <br> Escenario 2: Si no hay citas registradas, se muestra mensaje “No hay citas agendadas aún”.                              | EPIC01           |
...
| **Attendees (to planning meeting)** | Oscar Espinoza, Miguel Vidal, Stephano Landauri, Fabian Oliva, Gianfranco Durand, Erick Vasquez               |
| **Sprint 1 – 2 Review Summary** | Se finalizó correctamente la estructura visual y lógica del frontend, incluyendo rutas, vistas, y pruebas de navegación. La landing page fue completada con sus respectivas secciones funcionales. |
| **Sprint 1 – 2 Retrospective Summary** | Se identificó la necesidad de una mejor conexión entre frontend y backend. Se estableció priorizar endpoints REST en esta nueva fase, organizar mejor los contextos de negocio y reforzar testing de integración. |
