...
| HU03 | Elección y visualización de plan actual | Como dueño de mascota, quiero elegir un plan para que se visualice que plan actual tengo| Escenario 1: Ver la elección del plan actual **Given** ingreso a la sección de planes y visualizo la información, **When** elijo uno de los planes disponibles, **Then** debe mostrarse que plan actual se ha elegido. | EPIC02 |
...
| HU06 | Historial de pagos |Como dueño de mascota, quiero consultar un historial de pagos anteriores, para tener registro de mis transacciones realizadas. | Escenario 1: Luego de acceder a un pago se registra debajo un apartado del historial de pagos <br> **Given** que he pagado una cita o plan, **When** visualizo que debajo se registra el pago en un historial **Then** puedo verificar los datos del pago desde esa boleta. | EPIC02 |
| HU07 | Registro de mascota | Como dueño de mascota, quiero colocar información de mi mascota para registrarla | Escenario 1: Se desea registrar una mascota. <br> **Given** que quiero registrar a mi mascota, **When** ingreso su información **Then** puedo visualizar el registro de mi mascota. | EPIC03 |
| HU08 | Eliminar del registro a mascota | Como dueño de mascota, quiero cancelar el registro de mi mascota| Escenario 1: Se desea quitar del registro a una mascota. <br> **Given** que quiero eliminar a mi mascota del registro, **When** le doy al botón "Eliminar" **Then** mi mascota ha sido quitada del registro. | EPIC03 |
| HU09 | Comprar medicina | Como dueño de mascota, quiero comprar medicinas para el cuidado de mi mascota | Escenario 1: Quiero comprar medicina para mi mascota. <br> **Given** dado estoy viendo las medicinas, **When** agrego la cantidad que necesito **Then** se registra la compra en el historial | EPIC04 |
| HU10 | Modificar cantidad de medicina | Como dueño de mascota, quiero elegir la cantidad de medicinas para poder comprar solo lo que necesito | Escenario 1: Quiero elegir una cantidad de medicinas que comprar. <br> **Given** dado estoy viendo las medicinas, **When** elijo la cantidad que necesito **Then** se registra la compra en el historial de pedidos | EPIC04 |
| HU11 | Visualización de medicamentos comprados | Como dueño de mascota quiero ver una lista de mis medicamentos, para tener control sobre la atención de mi mascota. | Escenario 1: Se muestra una lista con los medicamentos ordenados por compra reciente<br>**Given** estoy autenticado, **When** accedo al historial", **Then** visualizo todos los medicamentos registrados. | EPIC05 |
| HU12 | Detalle de medicamentos | Como proveedor de cuidado animal, quiero ver el detalle del medicamento que se adquirió, para revisar la información específica. | Escenario 1: Al visualizar la lista de medicamentos se muestra el detalle sobre su efecto detallado. <br>**Given** estoy en el historial, **When** visualizo los medicamentos comprados, **Then** veo toda la información relacionada con la enfermedad que combate o efectos que realiza. | EPIC05 |
| HU13 | Historial de pedidos  | Como proveedor de cuidado animal,  quiero consultar el historial de todos los medicamentos pedidos, para hacer seguimiento del servicio ofrecido. | Escenario 1: Se muestra una lista completa de pedidos. <br>**Given** estoy autenticado, **When** accedo al historial, **Then** puedo ver toda¿os las pedidos de mis clientes con sus respectivos detalles. | EPIC05 |
| HU14 | Navegación por landing page | Como visitante del segmento objetivo dueño de mascota/proveedor de cuidado animal, quiero ver la información del landing para poder decidir si contratarlos| Escenario 1: Visualización del landing. <br> **Given** que el visitante del segmento objetivo dueño de mascotas ingresa a la landing **When** se carga la página, **Then** revisa la información y decide si contratar los servicios | EPIC06 |
| HU15 | Visualización de servicios ofrecidos | Como visitante, quiero ver tarjetas de servicios con iconos, nombre y descripción. Para poder comprender con facilidad | Escenario 1: Se muestran 4 tarjetas de servicios. <br> **Given** ingreso a la sección de Services, **When** se carga la página, **Then** visualizo las 4 tarjetas con información clara sobre cada uno de los servicios. | EPIC06 |
...
| HU17 |  Visualización de planes de suscripción | Como visitante del segmento objetivo dueño de mascota, quiero ver los tres planes para comparar cual es mejor opción | Escenario 1: Cada plan tiene una descripción sobre sus beneficios y botón "Read More". <br> **Given** estoy en la página de planes, **When** observo cada plan y sus beneficios, **Then** comparo entre que plan me beneficia más y elegirlo. | EPIC06 |
| HU18 |  Detalles de planes | Como visitante del segmento objetivo dueño de mascota, quiero visualizar a detalle los planes | Escenario 1: Cada uno de los planes tienen un botón "Read More". <br> **Given** estoy en la página de planes, **When** ingreso a detalles mediante el botón "Read More", **Then** se ma redirige a una página con detalles del plan seleccionado | EPIC06 |
| TS01 | Inicio de Sesión con validación | Como desarrollador, quiero que los usuarios ingresen su correo y contraseña y reciban una validación inmediata, para evitar campos vacíos o datos incorrectos.   | Escenario 1: El usuario deja campos vacíos y el sistema muestra una alerta “Por favor ingrese su correo y contraseña.”<br>**Given** completo el formulario de cita, **When** hago clic en "Confirmar", **Then** recibo una notificación visual de éxito. | EPIC01 |
...
| TS03 | Pago de planes | Como desarrollador, quiero mostrar los distintos métodos de pago, para que el usuario pueda pagar a su preferencia. | Escenario 1: El usuario elige un plan y seleccióna la opción de "Comprar" <br> **Given** que elige un plan y lo quiere comprar, **When** completa sus datos y elige un método de pago, **Then** se registra el pago y se guarda en historial de pagos | EPIC02 |
| TS04 | Historial de pagos | Como desarrollador, quiero mostrar los pagos realizados por los usuarios, para que el usuario pueda ver los pagos recientes que realizó. | Escenario 1: El usuario ya ha pagado y desea revisar su historial de pagos <br> **Given** que desea revisar la información de su último pago, **When** paga un plany visualiza debajo el historial de pagos , **Then** tiene la información reciente de su pago en el historial de pagos | EPIC02 |
| TS05 | Gestión de mascotas | Como desarrollador, quiero permitir que el usuario vea, agregue, elimine y seleccione mascotas en tarjetas, para que gestione fácilmente su información. | Escenario 1: El usuario accede a un formulario de registro de mascotas<br> **Given** que quiero gestionar las mascotas registradas, **When** uso las opciones de "Agregar" o "Eliminar", **Then** tengo un control de las mascotas registradas. | EPIC03 |
...
| TS07 | Compra de medicamentos | Como desarrollador, quiero mostrar medicamentos con nombre, descripción, selector de cantidad y botón “Comprar”, para que el usuario adquiera lo necesario. | Escenario 1: El usuario accede a la pestaña “Medicamentos” y ve vacunas, desparasitantes y antibióticos.<br>**Given** que accedo a mi perfil, **When** hago clic en “Historial”, **Then** veo una lista cronológica de mis servicios anteriores. | EPIC04 |
| TS08 | Validación de campos vacíos en compra | Como desarrollador, quiero validar que el usuario seleccione al menos una unidad de medicamento antes de hacer clic en “Comprar”, para evitar errores de compra. | Escenario 1: Si no hay unidades seleccionadas, el sistema muestra un mensaje de error.<br> **Given** que estoy en mi perfil, **When** hago clic en “Editar”, **Then** puedo actualizar mis datos y guardar los cambios correctamente. | EPIC04 |
| TS09 | Historial de medicamentos | Como desarrollador, quiero mostrar en una tabla el historial de productos aplicados a cada mascota, para que el usuario tenga trazabilidad de los tratamientos. | Escenario 1: El usuario accede al historial y ve una tabla con columnas: ID, Nombre, Mascota, Productos. <br> **Given** que estoy autenticado, **When** hago clic en “Cerrar sesión”, **Then** mi sesión se cierra y soy redirigido al login. | EPIC05 |
